...
promise_test(async (t) => {
  const abort_message = 'foo abort';
  let fetchPromise;
  let streamCancelPromise = new Promise(res => {
    ...
    fetchPromise = promise_rejects_exactly(t, abort_message, fetch('', {
                                             method: 'POST',
                                             body: stream,
                                             duplex: 'half',
  ...
  assert_equals(
      cancelReason, abort_message, 'ReadableStream.cancel should be called.');
